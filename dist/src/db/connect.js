...
async function connectDatabase() {
    const raw = config_1.CONFIG.DATABASE_URL;
    if (!raw) {
    ...
    if (!host)
        return { connected: false, message: "DATABASE_URL missing hostname (cannot verify reachability)" };
    try {
        if (!port || !Number.isFinite(port)) {
            return { connected: false, message: "DATABASE_URL missing port (cannot verify reachability)" };
        }
        await tcpPing(host, port, config_1.CONFIG.DB_CONNECT_TIMEOUT_MS);
        return { connected: true, message: "database is connected" };
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
