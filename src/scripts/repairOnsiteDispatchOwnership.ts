...
import { rebuildTicketLoads } from "../services/engineerAssignments";
...
function findDispatcherFromHistory(complaint: Complaint) {
  for (const event of [...(complaint.workflowHistory ?? [])].reverse()) {
    if (event.action !== "Assigned for onsite" || isL1Role(event.byRole)) continue;
...
async function main() {
  ...
  for (const complaint of complaints) {
    ...
    const ownershipClobbered = Boolean(onsiteEngineerName) && (
      recordedRequesterName === onsiteEngineerName || recordedAssignerName === onsiteEngineerName
    );
    if (!ownershipClobbered) continue;
    ...
    console.log(
      `REPAIR ${complaint.ticketNumber || complaint.id} (${complaint.status}): dispatcher ${normalizeText(complaint.siteVisitRequestedByName) || "unknown"} -> ${dispatcher.name}`
    );
    repaired += 1;
    ...
    if (complaint.status === "Escalated to L2" && normalizeLookup(complaint.assignedEngineerName) === onsiteEngineerName) {
      ...
      reassigned += 1;
      console.log(
        `       returned ticket reassigned ${normalizeText(complaint.assignedEngineerName)} -> ${dispatcher.name} for closure`
      );
    }
    ...
    await c.complaints.updateOne({ id: complaint.id }, update as any);
  }
