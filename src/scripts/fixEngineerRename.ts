...
async function main() {
  ...
  const role: EngineerRole = normalizedRole === "L2" ? "L2" : normalizedRole === "L3" ? "L3" : "L1";
  ...
  const result = await migrateEngineerIdentity({ oldName, newName, oldRole: role, newRole: role });
  ...
  const newId = engineerMasterId(newName, role);
