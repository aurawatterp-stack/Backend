...
/** GET /api/sales */
router.get("/", authenticate, requireAnyPermission("sales:entry", "dispatch:manage"), async (req: Request, res: Response) => {
...
router.put("/:id/dispatch-team", authenticate, requireAnyPermission("dispatch:manage"), async (req: Request, res: Response) => {
  ...
  if (dispatchStatus === "Final Dispatch" && !update.confirmedDispatchDate && !sale.confirmedDispatchDate) {
    return fail(res, "Confirm date of dispatch is required for final dispatch");
  }
  if (
    dispatchStatus === "Final Dispatch" &&
    !update.courierDocketNo &&
    !update.courierDocketAttachmentName &&
    !sale.courierDocketNo &&
    !sale.courierDocketAttachmentName
  ) {
    return fail(res, "Courier docket no. or docket attachment is required for final dispatch");
  }
