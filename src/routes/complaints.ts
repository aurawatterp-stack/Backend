 ...
 */
function resolveOnsiteReturnTarget(complaint: Complaint, actor: AuthUser) {
  const onsiteSideNames = new Set(
    [complaint.siteVisitEngineerName, complaint.engineerName, actor.name]
      .map(normalizeLookup)
      .filter(Boolean)
  );
  const isOnsiteSide = (id: string, name: string) => (
    (Boolean(id) && id === actor.userId) || onsiteSideNames.has(normalizeLookup(name))
  );

  const candidates: Array<{ id?: string; name?: string; role?: string }> = [
  ...
  for (const event of [...(complaint.workflowHistory ?? [])].reverse()) {
    if (event.action !== "Assigned for onsite" || isL1ServiceRole(event.byRole)) continue;
  ...
  for (const candidate of candidates) {
    ...
    if (!id && !name) continue;
    if (isOnsiteSide(id, name)) continue;
...
async function currentEngineerAssignmentFilters(user: AuthUser, role: "L1" | "L2" | "L3" | "Backup") {
  const ownMaster = await findEngineerMasterForUser(user, role);
  const or: Record<string, unknown>[] = [];
  const seen = new Set<string>();
  const add = (field: "assignedEngineerId" | "assignedEngineerName", value: unknown) => {
...
async function canAccessComplaint(user: AuthUser, complaint: Complaint): Promise<boolean> {
  ...
  if (user.role === "L1 Engineer" || user.role === "Backup") {
    const ownAssignmentFilters = await currentEngineerAssignmentFilters(user, "L1");
    const ownMatch = (
      ownAssignmentFilters.some((filter) => (
        ("assignedEngineerId" in filter && complaint.assignedEngineerId === filter.assignedEngineerId) ||
